...
final class TcpBridge {
    ...
    private volatile String bluetoothFeatures = ""; private volatile boolean mediaSendEnabled;
    ...
    boolean mediaReceiveAvailable() { Connection connection = activeConnection; return connection != null && connection.mediaReceiveAvailable(); }
    boolean sendMedia(ProtocolMessage message) { Connection connection = activeConnection; return connection != null && connection.sendMedia(message); }
    ...
    void setMediaSyncEnabled(boolean enabled) { if (mediaSendEnabled == enabled) return; mediaSendEnabled = enabled; final Connection connection = activeConnection; if (connection != null) new Thread(new Runnable() { @Override public void run() { connection.updateCapabilities(); } }, "explorer-media-capabilities").start(); }
    ...
    private final class Connection implements Runnable {
        private final Socket socket; private volatile boolean open = true; private LinkSession session; private volatile boolean peerPhoneActions, peerMediaReceive;
        ...
        synchronized boolean mediaReceiveAvailable() { return open && peerMediaReceive && session != null && session.authenticated(); }
        ...
        synchronized boolean sendMedia(ProtocolMessage message) { if (!mediaSendEnabled || !mediaReceiveAvailable() || !MediaTransfer.knownType(message.type)) return false; try { write(session.encrypt(message.type, message.payload)); return true; } catch (ProtocolException ignored) { close(socket); return false; } }
