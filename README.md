...
An optional [iPhone remote for the Mac installer](docs/PHONE-INSTALLER.md) controls the same checks, backup and preparation workflow. The Mac still hosts USB; it does not provide direct iPhone flashing. [Direct-install alternatives and remaining prerequisites](docs/IPHONE-FIRMWARE-OPTIONS.md) are documented separately.

The service can also be installed or updated as an APK on an existing compatible system. The Mac app bundles the service, USB tools, firmware, checksums, and instructions for personal transfer. Its guided setup checks Glass and its existing recovery, copies a verified backup to the Mac, then copies and verifies the bundled CWM files on Glass. It reports **Prepared** and guides the explicit restore on Glass; it never runs a restore or raw partition write. Unknown recovery or identity stops the workflow. No physical Glass writes or flashing validation have been performed. See [installation](docs/FLASHING.md) and [manufacturer/community safety findings](docs/COMMUNITY-SAFETY.md).

Notifications and media controls use Glass-side ANCS/AMS GATT clients. The iPhone app cannot read other apps' notifications or global Siri transcripts. A guarded Camera-button adapter requests Siri through the existing stock Glass HFP/SCO service while a companion card is open. Actual Siri playback and microphone operation require real-device testing; the original launcher's Camera binding is unchanged. The guarded adapter also supports ordinary APK installs on the audited stock system. "Hey Siri" wake-word detection is not implemented. See [headset research](docs/FEASIBILITY-HFP.md) and [capability research](docs/FEASIBILITY.md).

[Photo/video sync](docs/MEDIA-SYNC.md) transfers opted-in camera captures over authenticated Wi-Fi into the iPhone app's protected storage. Keep Explorer Link open while receiving; preview files and choose **Save to Photos** later. Transfers verify lengths and hashes, detect duplicates and retain Glass originals. Real Glass camera discovery and transfer still require hardware testing.
